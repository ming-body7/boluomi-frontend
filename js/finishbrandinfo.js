...
var server = {
	submit_audit : function(id,auth_key,name,phone,logo,type,province,city,area,address,licence,location,callback){
		...
		var data = {status:0,id:id,auth_key:auth_key,name:name,phone:phone,logo:logo,type:type,province:province,city:city,area:area,address:address,licence:licence,location:location};
...
var handler = {
	...
	rander_load_img : function(par){
		...
		upLoadPic.uploadify({
			...
			"uploader" : 'http://182.92.194.42:8083/upload.php',
			"width" : 120,
			"height" : 30,
			"fileObjName":'upfile',
	...
	submit_audit_click : function(){
		...
		var city = $obj.J_city.find('option:checked').html() == '请选择' ? '未填' : $obj.J_city.find('option:checked').html();
		var area = $obj.J_area.find('option:checked').html() == '请选择' ? '未填' : $obj.J_area.find('option:checked').html();
...
;(function(){
	...
	{ 	
		 ...
		 //    var imgname=$( "#imgname" ).val().trim();
			// var imgtype=$( "#imgtype" ).val().trim(); 
	   ...
	   $.ajaxFileUpload
	      (
	        {
	             url:'http://182.92.194.42:8083/upload.php', //你处理上传文件的服务端
	             secureuri:true,
	             fileElementId:'file',
	             dataType: 'json',
	             type : 'POST',
